SwitchUser.setup do |config|
  ...
  config.available_users = { user: -> { User.where(id: Settings.anonymous_number) } }
  ...
  config.controller_guard = ->(current_user, request) { Rails.env.development?  || Rails.env.production? }

  # view_guard is a block,
  # if it returns true, the switch user select box will be shown,
  # else the select box will not be shown
  # if you switch from admin to "user", the current_user param is "user"
  config.view_guard = ->(current_user, request) { Rails.env.development? || Rails.env.production?}
